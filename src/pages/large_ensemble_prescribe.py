...
from src.pages.prescriptor.neat_multi.neat_prescriptor_many_objective import Neat as Neat2D
# from ongoing.prescriptors.neat_13D.neat_prescriptor_many_objective import Neat as Neat13D
from src.pages.prescriptor.bandit.bandit_prescriptor import Bandit
from src.pages.prescriptor.bandit_v2.bandit_v2_prescriptor import BanditV2
from src.pages.prescriptor.heuristic.heuristic_prescriptor import Heuristic
from src.pages.prescriptor.blind_greedy.blind_greedy_prescriptor import BlindGreedy
from src.pages.prescriptor.xprize_predictor import NPI_COLUMNS, XPrizePredictor
import src.pages.prescriptor.base as base

ROOT_DIR = "src/pages/prescriptor/"
...
# BanditV2 configs
BANDIT_V2_PATH = os.path.join(ROOT_DIR, 'bandit_v2')
BANDIT_V2_FILE = os.path.join(BANDIT_V2_PATH, 'bandit_params.pkl')
...
PREDICTOR_PATH = 'src/models/trained_model_weights.h5'
DATA_DIR = 'src/data/'
...
def prescribe(start_date_str: str,
    ...
    geos = hist_df['GeoID'].unique()

    # Load the IP weights
    cost_df = pd.read_csv(path_to_cost_file)
    cost_df = base.add_geo_id(cost_df)
    cost_df = cost_df[cost_df['GeoID'].isin(geos)]
    geos = cost_df['GeoID'].unique()

    # Discard countires that we have no historical data or costs for
    npi_df = npi_df[npi_df['GeoID'].isin(geos)]
    ...
    bandit = BanditV2(hist_df=hist_df, model_fname=BANDIT_V2_FILE)
    neat2d_ad14_ckp0 = Neat2D(prescriptors_file=NEAT2D_FILE_0, hist_df=hist_df, config_file=NEAT2D_CONFIG_FILE_0, action_duration=14)
    neat2d_ad14_ckp1 = Neat2D(prescriptors_file=NEAT2D_FILE_1, hist_df=hist_df, config_file=NEAT2D_CONFIG_FILE_1, action_duration=14)
    ...
    prescriptors = {
        ...
        'Neat2D_AD14_CKP0': neat2d_ad14_ckp0,
        'Neat2D_AD14_CKP1': neat2d_ad14_ckp1,
