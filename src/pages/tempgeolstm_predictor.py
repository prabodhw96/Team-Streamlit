...
from src.pages.base import BasePredictor

DATA_URL = "https://raw.githubusercontent.com/OxCGRT/covid-policy-tracker/master/data/OxCGRT_latest.csv"

TEMPERATURE_DATA_FILE_PATH = "src/data/temperature_data.csv"
...
class tempGeoLSTMPredictor(BasePredictor):
    ...
    """
    def __init__(self, path_to_model_weights=None, path_to_geos=None, use_embedding=True, embed_size=EMBED_SIZE, lstm_size=LSTM_SIZE, nb_lookback_days=NB_LOOKBACK_DAYS, nb_test_days=NB_TEST_DAYS, window_size=WINDOW_SIZE, npi_delay=NPI_DELAY, num_trials=NUM_TRIALS, num_epochs=NUM_EPOCHS, seed=base.SEED):
        ...
        if path_to_model_weights:
            ...
            if self.use_embedding:
                ...
                self.geos = None
            self.predictor, _ = self._construct_model(nb_context=nb_context, nb_action=nb_action, embed_size=self.embed_size, lstm_size=self.lstm_size, nb_lookback_days=self.nb_lookback_days)
    ...
    def predict(self, data=None, start_date=None, end_date=None):
        ...
        self.country_samples = self._create_country_samples(train_df, list(self.train_df.GeoID.unique()), self.nb_lookback_days, self.npi_delay, self.nb_test_days)
    ...
    def _get_new_cases_preds(self, c_df, g, npis_df, temp_df):
        ...
        preds = self._roll_out_predictions(self.predictor, initial_context_input, initial_action_input, country_id, npis_sequence, temp_sequence, holiday_sequence)
        preds_cases = preds[:,0]
        ...
        pred_total_cases, pred_new_cases = base.convert_ratios_to_total_cases(preds_cases, self.window_size, prev_new_cases, initial_total_cases, pop_size)

        # Compute predictor's deaths forecast
        pred_total_deaths, pred_new_deaths = base.convert_ratios_to_total_deaths(preds_deaths, self.window_size, prev_new_deaths, initial_total_deaths)
    ...
    def fit(self):
        ...
        self.country_samples = self._create_country_samples(train_df, list(train_df.GeoID.unique()), self.nb_lookback_days, self.npi_delay, self.nb_test_days)
        self.geos = list(self.country_samples.keys())

        # Aggregate data for training
        all_X_context_list = [self.country_samples[c]['X_context'] for c in self.country_samples]
        all_X_action_list = [self.country_samples[c]['X_action'] for c in self.country_samples]
        all_X_country_list = [self.country_samples[c]['X_country'] for c in self.country_samples]
        all_y_list = [self.country_samples[c]['y'] for c in self.country_samples]
        ...
        self.predictor, training_model = self._construct_model(nb_context=X_context.shape[-1], nb_action=X_action.shape[-1], embed_size=self.embed_size, lstm_size=self.lstm_size, nb_lookback_days=self.nb_lookback_days)
