...
from src.pages.prescriptor.base import BasePrescriptor, NPI_COLUMNS, NPI_MAX_VALUES
        ...
        for pidx, weight in enumerate(self.weights):
            ...
            for _ in range(self.nb_iter):
                for geo in eval_geos:
                    ...
                    one_step_bfr_df = df[df['Date'] == self.eval_start_date - np.timedelta64(self.action_duration, 'D')]
                    two_step_bfr_df = df[df['Date'] == self.eval_start_date - np.timedelta64(2*self.action_duration, 'D')]
                    ...
                    while action_start_date <= self.eval_end_date:
                        ...
                        new_pred_df['Population'] = one_step_bfr_df['Population'].to_numpy()
                        for i, npi in enumerate(NPI_COLUMNS):
                            new_pred_df[npi] = npi_df[npi]

                        case_ratio = (one_step_bfr_df['SmoothNewCases'].to_numpy()/max(1., new_pred_df['SmoothNewCases'].to_numpy()))[0]
                        ...
                        two_step_bfr_df = one_step_bfr_df
                        one_step_bfr_df = new_pred_df
        ...
        for pidx in range(len(self.weights)):
            ...
            while action_start_date <= end_date:
                ...
                for geo in geos:
                    ...
                    # if one_step_bfr_df.empty or two_step_bfr_df.empty:  # if no historical data available for this geo, prescribe 0s
                    #     actions = np.zeros(len(NPI_COLUMNS))
                    #     country_name = cost_df[cost_df['GeoID'] == geo]['CountryName'].to_list()[0]
                    #     region_name = cost_df[cost_df['GeoID'] == geo]['RegionName'].to_list()[0]
                    #     for date in pd.date_range(action_start_date, action_end_date):
                ...
                action_start_date += pd.Timedelta(days=self.action_duration)
