...
#from src.pages.prescriptor.neat_13D.neat_prescriptor_many_objective import Neat as Neat13D
...
def prescribe(start_date_str: str, end_date_str: str, prior_ip_df, cost_df_file) -> None:
    npi_df = prior_ip_df #pd.read_csv(path_to_prior_ips_file, parse_dates=['Date'], encoding="ISO-8859-1", error_bad_lines=True)
    npi_df = base.add_geo_id(npi_df)

    hist_df = base.load_historical_data(update_data=False)

    cost_df = cost_df_file
    ...
    neat2d_ad15_ckp0 = Neat2D(prescriptors_file=NEAT2D_FILE_0, hist_df=hist_df, config_file=NEAT2D_CONFIG_FILE_0, action_duration=14)
    neat2d_ad15_ckp1 = Neat2D(prescriptors_file=NEAT2D_FILE_1, hist_df=hist_df, config_file=NEAT2D_CONFIG_FILE_1, action_duration=14)
    ...
    for presc_name, presc in prescriptors.items():
        try:  # in case anything goes wrong with this prescriptor, ignore it and move to the next one
            ...
            agg_pres_df = pres_df.groupby(['CountryName', 'PrescriptionIndex']).mean().reset_index() #'RegionName' dropna=False
